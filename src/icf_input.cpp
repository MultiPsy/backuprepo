...
/* find the switch and its arg......                                         */
   i = ICF::Set_SinIntArg ((char *)e_ICF_RAWS,&i_Arg); /* WEATHER_DATA:      */   // reads the RAWS value
   if ( i == -1 )
...
/* Alloc structs, might not use all if there is any Feb 29 data */
  this->a_RAWS = new d_RAWS[i_Arg + 1];  /* 1 extra for projected wethr rec, see below */     // i_Arg = the number of RAWS line
  if ( this->a_RAWS == NULL ) {
...
void Show_WeatherData(int index1) {
