	...
	grounddata& gfuel, long* posit)
{
	...
	//printf("GetCellPositioncount = %d\n", getcellpositioncnt1);
	//printf("GetCellPosition\n");
	if (!CantAllocLCP)
	{
		//printf("!CantAllocLCP\n");
	...
	{
		printf("Header.CrownFuels == 20\n");
		if (Header.GroundFuels == 20)
		{
			printf("Header.GroundFuels == 20\n");
...
void Farsite5::ReadHeader()
{
	...
	//for(int i = 0; i < 100; i++)	{								// fuel models 종류 확인 가능
	//	printf("Header.fuels[%d] = %d\n", i, Header.fuels[i]);
	//}
...
*/
	if (HaveCrownFuels())
	{
		if (HaveGroundFuels())
			...
			NumVals = 8;				// NumVals = 8 - lcp file 8 bands?
	}
	else
	{
		if (HaveGroundFuels())
	...
	lcptheme = new LandscapeTheme(false, this);		// landscpaetheme -> gridtheme
													// NumCats, MaxVal, MinVal
	...
	if (landscape == 0)
	{
		...
		if (CantAllocLCP == false)
		{
			//printf("CantAllocLCP == false\n");
			long i;
			double NumAlloc;

			fseek(landfile, headsize, SEEK_SET);		// 파일 포인터를 header 파일 끝으로 (lcp header 파일의 크기는 7316 byte)
			//     		if((landscape=(short *) calloc(Header.numnorth*Header.numeast, NumVals*sizeof(short)))!=NULL)
			NumAlloc = (double)(Header.numnorth * Header.numeast * NumVals * sizeof(short));		// file size * 2 (why?)
			//printf("Header.numnorth = %d, Header.numeast = %d, NumVals = %d\n", Header.numnorth, Header.numeast, NumVals);
			//printf("NumAlloc = %lf\n", NumAlloc);
			...
			try
			{
				landscape = new short[Header.numnorth * Header.numeast * NumVals];	// 8 bands의 셀들에 들어가는 value들을 저장할 배열 선언
			}
			catch (...)
			{
				landscape = 0;
			...
			{
				//ZeroMemory(landscape,Header.numnorth * Header.numeast * NumVals * sizeof(short));
                memset(landscape,0x0,Header.numnorth * Header.numeast * NumVals * sizeof(short));	// landscape 배열 0x0으로 초기화
				for (i = 0; i < Header.numnorth; i++)	{	// landfile에서 NumVals * Header.numeast만큼의 values를 landscapes에 저장
					fread(&landscape[i * NumVals * Header.numeast], sizeof(short), NumVals * Header.numeast, landfile);
				}
				fseek(landfile, headsize, SEEK_SET);	// landfile 파일 포인터를 헤더 다음으로 이동
				//     		 	OldFilePosition=0;    	// thread local
				CantAllocLCP = false;
			}
			else
	...
	/*for(int i = 0; i < NumVals * Header.numeast * Header.numnorth; i++)	{
		printf("%hd\n", landscape[i]);												// landscape 배열 출력	
	}*/	
...
long Farsite5::GetCellPosition(double east, double north)
{
	...
	if (northi < 0)
		northi = 0;
	long posit = (northi* Header.numeast + easti);				// 748 * 648 = 484704에서 cell이 위치하는 element value
	//printf("xpt = %lf, ypt = %lf\n", xpt, ypt);
	//printf("posit = %ld\n", posit);

	return posit;
}

long Farsite5::GetCellPosition2(double east, double north)
{
	//getcellpositioncnt1++;
	//printf("GetCellPosition()\n");
	double xpt = (east - Header.loeast) / GetCellResolutionX();
	...
	if (northi < 0)
		northi = 0;
	long posit = (northi* Header.numeast + easti);				// 748 * 648 = 484704에서 cell이 위치하는 원소 value
	//printf("xpt = %lf, ypt = %lf\n", xpt, ypt);
	printf("posit = %ld\n", posit);
...
{
//	WaitForSingleObject(hNewSpotSemaphore, INFINITE);
	//printf("<Farsite5.cpp> IncNewFires()---------------------------------------------\n");
	newfires += increment;
	numspots = newfires;
	//printf("newfires = %ld\n", newfires);
//	ReleaseSemaphore(hNewSpotSemaphore, 1, &prevct);
...
{
	//printf("<Farsite5.cpp> DistanceCheckMethod()---------------------------------------------\n");
	if (Method >= 0)
		DistanceMethod = Method;

	//printf("<Farsite5.cpp> DistanceCheckMethod() Method: %ld, DistanceMethod: %ld\n", Method, DistanceMethod);
...
{
	//printf("<Farsite.5.cpp> GetPerimeter2Value()---------------------------------------------\n");
	if (coord < 0 || value < 0)
...
double* Farsite5::AllocPerimeter1(long NumFire, long NumPoints)
{
	//WaitForSingleObject(hNewPerimSemaphore, INFINITE);
	if (NumPoints)
	{	//cout << "<Farsite5.cpp> AllocPerimeter1()" << endl;
...
void Farsite5::SetPerimeter1(long NumFire, long NumPoint, double xpt, double ypt)
{
	//	WaitForSingleObject(hNewPerimSemaphore, INFINITE);
	NumPoint *= NUMDATA;
	//printf("NumPoint = %ld, NumPoint+1 = %ld\n", NumPoint, NumPoint+1);
...
int Farsite5::LoadInputsFile(char *FileName)
{
    ...
    if(strlen(icf.cr_FarsiteIgnition) > 0)  {
		//printf("strlen(icf.cr_FarsiteIgnition) > 0\n");
...
{
  //printf("Farsite5.cpp <LoadWind_RAWS> ------------------------------\n");
int i, i_Mth, iX;
long  StationNumber;
d_RAWS *a;
...
{
	//printf("<Farsite5.cpp> AllocStationGrid()---------------------------------------------\n");
	long x, y, Num;
	...
	for (y = 0; y < YDim; y++)
	{
		...
		{
			Num = x + y * grid.XDim;
			//printf("<Farsite5.cpp> AllocStationGrid() Num = %ld\n", Num);
			grid.Grid[Num] = 1;
		}
	}
}
...
{
	//cout << "<Farsite5.cpp> FarsiteSimulationLoop()---------------------------------------------" << endl;
	...
	//cout << "<Farsite5.cpp> FarsiteSimulationLoop() icf.f_FarsiteSpotGridResolution = " << icf.f_FarsiteSpotGridResolution << endl;
...
/*.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-..-*/
	...
	//while (burn.SIMTIME <= 180)	{
	//while (burn.SIMTIME <= 120)	{
	//while (burn.SIMTIME <= 60)	{
	//while (burn.SIMTIME <= 0)	{
		//printf("\n");
		//printf("*--------------------------------------------------------------------------------------------*\n");
		...
		if (FARSITE_GO)		{
	  		if (PreCalcMoistures(GETVAL)) {
        		if ( CondSw == 0 ) {             /* Do once to do all conditioning */
					...
					//printf("<Farsite5.cpp> FarsiteSimulationLoop() CondSw = %d\n", CondSw);
        		}
...
//			 	burn.env->CalcMapFuelMoistures(burn.SIMTIME +	GetActualTimeStep());
		...
		//printf("GetNumFIres() = %ld\n", GetNumFires());
		//cout << "\nLoop elapsed time:" << (end.tv_sec - begin.tv_sec) + (end.tv_nsec - begin.tv_nsec) / 10000000000.0 << endl;
		//printf("BurnItcnt = %d\n", BurnItcnt);		
		//printf("loopCnt = %d\n", loopCnt);
		//printf("*--------------------------------------------------------------------------------------------*\n\n\n");
		BurnItcnt = 0;
		...
		//printf("Month = %d\n", icf.a_RAWS[loopCnt].i_Mth);
		//printf("Day = %d\n", icf.a_RAWS[loopCnt].i_Day);
		//printf("Time = %d\n", icf.a_RAWS[loopCnt].i_Time);
		loopCnt++;
	}  /* 	while (burn.SIMTIME <= maximum)	*/

	// print out hourly weather data
	//cout << "i_Mth = " << icf.a_RAWS[107].i_Mth << endl;

	//printf("simulation maximum time = %ld\n", maximum);
	

	//d_weather = new d_RAWS[loopCnt];
...
void Farsite5::ShowPerimeterCoords()	{
...
{
	//printf("<Farsite5.cpp> FarsiteProcess1()---------------------------------------------\n"); // called only one time
	long InOut, FireNum;

	//printf("NextFireAfterInterrupt = %ld\n", NextFireAfterInterrupt);
	IN_BURNPERIOD = true;
	if (DistanceCheckMethod(GETVAL) == DISTCHECK_SIMLEVEL)  	  // if simlevel timestep and/or burnout enabled
	{
		//printf("<Farsite5.cpp> FarsiteProcess1() DistanceCheckMethod() == DISTCHECK_SIMLEVEL\n");
		if (NextFireAfterInterrupt == 0)
		...
		for (CurrentFire = NextFireAfterInterrupt; CurrentFire < GetNumFires(); CurrentFire++)			// for all fires
		{
			...
			//printf("111FarsiteProcess1() BurnIt++\n");
			BurnItcnt++;
	...
	{
		//printf("<Farsite5.cpp> FarsiteProcess1() DistanceCheckMethod() else\n");
		...
		{
			//printf("<Farsite5.cpp> FarsiteProcess1() DistanceCheckMethod() NextFireAfterInterrupt == 0\n");
			a1 = 0.0;
			p1 = 0.0;
			p2 = 0.0;
		}
		for (CurrentFire = NextFireAfterInterrupt; CurrentFire < GetNumFires();CurrentFire++)	{ // for all fires
			//printf("GetNumFires() = %ld\n", GetNumFires());
			if ((InOut = GetInout(CurrentFire)) == 0)	// InOut == 현재번째 불에 해당하는 원소
				continue;
			//if(GetInout(CurrentFire)<3)   		// no outputs of vector barriers
			if (OutputVectsAsGrown) {
				WriteVectorOutputs();   		   // output perimeters to screen and/or vector file
			}
			//printf("CurrentFire = %ld\n", CurrentFire);
			burn.BurnIt(CurrentFire);   			// burn the fire
			//printf("FarsiteProcess1() BurnIt++\n\n");
			BurnItcnt++;
			//printf("<Farsite5.cpp> FarsiteProcess1() DistanceCheckMethod() InOut = %ld\n", InOut);
			if (InOut < 3)
			{
				a1 += burn.prod.area;   			// +area of all fires (&& -inward fire area)
				p1 += burn.prod.perimeter;  		// +perimeter of all fires)
...
bool Farsite5::FarsiteProcessSpots()	{
	// THIS FUNCTION CONTAINS THE FARSITE PROCESS CONTROL
	//printf("<Farsite5.cpp> FarsiteProcessSpots()---------------------------------------------\n");
	...
	//printf("burn.TotalSpots = %ld\n", burn.TotalSpots);
	for (CurrentFire = 0; CurrentFire < burn.TotalSpots; CurrentFire++)	{ // for all fires
		...
		CurrentSpot = InsertSpotFire(curspot->x, curspot->y);		// spotfire coordinates
		//cellx = (long)((curspot->x-Header.loeast)/res);
		//celly = (long)((curspot->y-Header.lonorth)/res);
		//printf("Current Spotfire coordinates x = %lf, y = %lf\n", curspot->x, curspot->y);
		//printf("and the cell index = %ld, %ld\n", cellx, celly);
		if (CurrentSpot < 0)
			continue;
		...
		//printf("FarsiteProcessSpots() BurnIt++\n");
		BurnItcnt++;
		if (InOut < 3)
		{
			a1 += burn.prod.area;   			// +area of all fires (&& -inward fire area)
...
{
	//printf("<Farsite5.cpp> FarsiteProcess2()---------------------------------------------\n");
	if (!IN_BURNPERIOD)
	{
		ProcNum = 3;
...
{
	//printf("<Farsite5.cpp> WriteVectorOutputs() ----------\n");
...
{
	//printf("<Farsite5.cpp> Execute_StartRestart()---------------------------------------------\n");
	int i_Ret;
	...
	/*     if(DDE.DDE_SIMCOMMAND)   	// delay response for dde return
			...
			*/
	//cout << "SIMULATE_GO :" << SIMULATE_GO << endl;
	if (SIMULATE_GO)
	{
		//Beep(24000, 150);
	...
	{
		//printf("<Farsite5.cpp> Execute_StartRestart() SIMULATE_GO == 0\n");
		if (F_ON)
		{
			long i, RealFires = 0;
			...
			if (RealFires > 0)	//GetNewFires()>0
			{
				...
				else
				{
					...
					if (!NEEDMX)
					{
						//printf("<Farsite5.cpp> Execute_StartRestart() if(!NEEDMX)\n");
					...
					clock_gettime(CLOCK_MONOTONIC, &end);
					//cout << "\nFarsiteSimulationLoop time:" << (end.tv_sec - begin.tv_sec) + (end.tv_nsec - begin.tv_nsec) / 10000000000.0 << endl;
...
{
	//printf("<Farsite5.cpp> LaunchFarsite()---------------------------------------------\n");
	struct timespec begin, end;
	
	int i_Ret;
	if(!Ignition.GetLightsLandscape())
		return e_EMS_FARSITE_NO_IGNITION;
   Ignition.ShapeInput();					// shapefile의 polygon에 대한 산불 개수, 발화점 좌표, 산불 확산 파라미터 설정, bounding box 설정
   if(strlen(icf.cr_FarsiteBarrier) > 0)
   {
	...
	clock_gettime(CLOCK_MONOTONIC, &end);
	//cout << "\nExecute_StartRestart time:" << (end.tv_sec - begin.tv_sec) + (end.tv_nsec - begin.tv_nsec) / 10000000000.0 << endl;
...
int Farsite5::CreateIgnitionGrid()
{
	...
	//printf("ignitionRows = %ld, ignitionCols = %ld\n", ignitionRows, ignitionCols);
	...
	/*for(int i = 0; i < ignitionRows; i++)	{
		for(int j = 0; j < ignitionCols; j++)	{
			igCell = CellData(i, j, igCell, igCrown, igGround, &garbage);
			printf("%hd ", igCell.f);
		}
		printf("\n");
	}*/
		...
		long i, j, NumPts, NumAlloc, NumSeg, cellx, celly;
		double x, y, x1, x2, y1, y2, dx, dy, dist, fract, res, linedist;
		res = Header.XResol;								// 해상도 30m
		//printf("res = %lf\n", res);
		dist=res/IGNITON_GRID_LINEDIST_DIVISOR;				// km 단위로 변환 -> 0.03km 
		//printf("dist = %lf\n", dist);
		double tWest = GetLoEast();
		//double tSouth = GetLoNorth();
		double tNorth = GetHiNorth();
		//printf("tWest = %lf, tNorth = %lf\n", tWest, tNorth);
		//westUTM = pFarsite->ConvertEastingOffsetToUtm(West);//*MetersToKm);
			//fprintf(otpfile, "%s %lf\n", "YLLCORNER",
		//southUTM = pFarsite->ConvertNorthingOffsetToUtm(South);//*MetersToKm);
		...
		//printf("west = %lf, north = %lf\n", west, north);
		long NumCols = ignitionCols, NumRows = ignitionRows;
		//printf("NumCols = %ld, NumRows = %ld\n", NumCols, NumRows);
		SHPHandle	hSHP;
		double 	adfMinBound[4], adfMaxBound[4];
		SHPObject *pShape;
		int nShapes, shapeType;
		...
		SHPGetInfo( hSHP, &nShapes, &shapeType, adfMinBound, adfMaxBound );
		NumAlloc=0;
		/*for(int i = 0; i < 4; i++)	{		// 최좌측(min), 최우측(max) 점의 좌표
			printf("adfMinBound[%d] = %lf, adfMaxBound[%d] = %lf\n", i, adfMinBound[i], i, adfMaxBound[i]);
		}*/
		//printf("nShapes = %d\n", nShapes);
		for(int s = 0; s < nShapes; s++ )
		{
			...
			{
				xPolygon poly;
				for(int p = 0; p < pShape->nParts; p++)					// 도형의 개수
				{
					//printf("\n%d th polygon\n", p + 1);
					//printf("pShape->nParts = %d\n", pShape->nParts);
					int pEnd = (p < pShape->nParts - 1) ? pShape->panPartStart[p + 1] : pShape->nVertices;
					//printf("pShape->panPartStart[p+1] = %d\npShape->nVertices = %d\n", pShape->panPartStart[p + 1], pShape->nVertices);
					//printf("pEnd = %d\n", pEnd);
					NumPts = pEnd - pShape->panPartStart[p];// + 1;
					//printf("NumPts = %ld\n", NumPts);					// polygon 꼭지점 수 + 1
					if(pts)
						delete[] pts;
					pts=new double[NumPts*2];							// polygon 꼭지점 좌표를 저장하는 배열
					i = 0;
					for(int v = pShape->panPartStart[p]; v < pEnd; v++)
					{
						pts[i*2] = pShape->padfX[v];					// x 좌표
						pts[i*2+1]= pShape->padfY[v];					// y 좌표
						i++;											// i == 짝수: x 좌표, i == 홀수: y 좌표
					}
					/*for(int v = 0; v < NumPts*2; v++)	{				// 꼭지점의 좌표
						printf("pts[%d] = %lf\n", v, pts[v]);
					}*/
					// copy zeros to cells crossed by line
					x1=pts[0];
					y1=pts[1];
					//printf("x1 = %lf, y1 = %lf\n\n", x1, y1);			// polygon 시작 꼭지점 좌표 출력
					poly.AllocVertex(NumPts);
					poly.SetVertex(x1, y1, 0);							// polygon 꼭지점 설정
					for(i=1; i<NumPts; i++)								// 꼭지점의 개수만큼 반복
					{
						x2=pts[i*2];
						y2=pts[i*2+1];
						//printf("x2 = %lf, y2 = %lf\n", x2, y2);			// polygon 꼭지점 좌표
						poly.SetVertex(x2, y2, i);						// 꼭지점을 polygon의 꼭지점으로 설정
						dx=(x1-x2);
						dy=(y1-y2);
						linedist=sqrt(pow2(dx)+pow2(dy));
						//printf("linedist = %lf\n", linedist);
						NumSeg=(long) (linedist/dist)+1;				// Meaning of NumSeg?
						//printf("NumSeg = %ld\n", NumSeg);
						fract=dist/linedist;	//((double) NumSeg);	// fract meaning?
						count1 = 0;
						for(j=0; j<NumSeg; j++)
						{
							x=x1-dx*fract*((double) j);
							...
							//}
							//printf("%hd  ", igCell.f);						// 30m 거리마다의 cell fuel model
							count1++;
						}						
						//printf("x1 = %lf, y1 = %lf\n", x1, y1);
						x1=x2;
						...
						//printf("x1 = %lf, y1 = %lf\n", x1, y1);
						//printf("\n");						
						//printf("ignition polygon coordinates\ncellx = %ld, celly = %ld\n", cellx, celly);			// polygon 꼭지점 cell 좌표
						//printf("count = %d\n", count1);
					}
					FillIgnitionPolygon(&poly, 1.0); // Fills polygon with 1's -> 1.0 = 다 탄 부분
...
void Farsite5::ShowRasterFuels()	{
	//cout << "ShowRasterFuels()" << endl;
	FILE *fp;
	...
	grounddata igGround;
	long garbage;
	double x1, y1 = Header.lonorth;

	fp = fopen("fuels.txt", "w");
	for(int i = 0; i < Header.numnorth; i++)	{
		x1 = Header.loeast;
		for(int j = 0; j < Header.numeast; j++)	{
			//printf("%lf.0\n", x);
			igCell = CellData(x1, y1, igCell, igCrown, igGround, &garbage);
			x1 += 30;
			//printf("%hd ", igCell.f);
			fprintf(fp, " %hd", igCell.f);
		}
		y1 += 30;
		fprintf(fp, "\n");
		//printf("\n");
...
{
	//printf("<Farsite5.cpp> AddCurrPerimeter() ----------\n");
...
{
	//printf("<Farsite5.cpp> AddIgnitionToSpotGrid()---------------------------------------------\n");
	if(!ignitionGrid)
		return 0;
	if(!m_BSG.Grid)
		return 0;
	...
	//printf("<Farsite5.cpp> AddIgnitionToSpotGrid() res = %lf\n", res);
	//long row, col;
