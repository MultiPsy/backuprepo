...
int  FE2::CondMngr ()
{
  int CalcMapFuelMoisturescnt = 0;
  //printf("<FMC_FE2.cpp> CondMngr()---------------------------------------------\n");
...
/* Run simuation for each time increment */
     ...
     CalcMapFuelMoistures(MoistSimTime);    // 시뮬레이션 수행 시 첫 번째 루프에서 가장 많은 딜레이가 발생하는 함수
     //printf("CalcMapFuelMoisturescnt = %d\n", ++CalcMapFuelMoisturescnt);
		 d_Progress = MoistSimTime / MaxTime;  /* Percent completed */
     if ( d_Progress > 1.0 )               /* might go a little over */
       d_Progress = 1.0;

     //printf("<FMC_FE2.cpp> CondMngr() MoistSimTime = %lf\n", MoistSimTime);     // MoistSimTime: .input 파일에 정의된 RAWS time += 60
     //printf("<FMC_FE2.cpp> CondMngr() MaxTime = %lf\n", MaxTime);               // MaxTime: .input 파일에 정의된 최종 RAWS time
  }  while ( MoistSimTime < MaxTime);      // MaxTime is class variable

  //printf("<FMC_FE2.cpp> end of do while loop---------------------------------------------\n");
...
// test-larry
  ...
  //printf("<FMC_FE2.cpp> end of CondMngr()---------------------------------------------\n");
...
{
  //printf("<FMC_FE2.cpp> CalcMapFuelMoistures()---------------------------------------------\n");
...
/* Do Daily Weather Data or Hourly RAWS data, depending on what was in the inputs */
   ...
   //printf("<FMC_FE2.cpp> end of CalccMapFuelMoistures()---------------------------------------------\n");
