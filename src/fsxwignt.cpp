...
{
	//printf("<fsxwignt.cpp> ShapeInput()---------------------------------------------\n");
	SHPHandle hSHP;
	SHPObject* pSHP;
	...
	//printf("<fsxwignt.cpp> ShapeInput() nEntities = %d\n", nEntities);
	for (i = 0; i < nEntities; i++)
	{
		...
		case SHPT_ARCZ:
		case SHPT_ARCM:
			//printf("case SHPT_ARCM\n");
			//printf("pSHP->nParts = %d\n", pSHP->nParts);
		...
		case SHPT_MULTIPATCH:
			//printf("<fsxwignt.cpp> ShapeInput() case SHPT_MULTIPATCH\n");
			//printf("<fsxwignt.cpp> ShapeInput() The number of fire polygons(pSHP->nParts) = %d\n", pSHP->nParts);
			for (m = 0; m < pSHP->nParts; m++)				// 발화점 polygon의 개수 == fire의 수 (NewFire)
			{
				NewFire = pFarsite->GetNewFires();
				//printf("NewFire = %ld\n", NewFire);
				start = pSHP->panPartStart[m];
				//printf("start = %ld\n", start);
				if (m < pSHP->nParts - 1)
					end = pSHP->panPartStart[m + 1];	// end == polygon 꼭지점 개수 + 1
				else
					end = pSHP->nVertices;
				//printf("end = %ld\n", end);
				if ((end - start) <= 0)
				...
				//printf("end of AllocPerimeter1() fsxwignt.cpp\n");
				for (j = start; j < end; j++)			// polygon 좌표 설정
				{
					xpt = pFarsite->ConvertUtmToEastingOffset(pSHP->padfX[j]);
					ypt = pFarsite->ConvertUtmToNorthingOffset(pSHP->padfY[j]);
					pFarsite->SetPerimeter1(NewFire, j - start, xpt, ypt);				// polygon 배열 좌표 입력
					//printf("11\n");
					pFarsite->SetFireChx(NewFire, j - start, -1.0, 0.0);				// rate of spread, fireline intensity 설정
					pFarsite->SetReact(NewFire, j - start, 0.0);						// fire reaction??
				}
				pFarsite->SetNumPoints(NewFire, end - start);							// NewFire가 가리키는 산불에 대해 꼭지점 수를 설정
				pFarsite->SetInout(NewFire, 1);											// NewFire의 불이 있는지 여부를 확인하는 함수? ( != 0 )
				pFarsite->IncNewFires(1);												// 따라서 새로운 불 수 + 1
				if (arp() < 0)
					ReversePoints(1);
				BoundingBox(NewFire);	  		// find bounding box for line source    // what is line source??
			}
			break;
		}
		SHPDestroyObject(pSHP);
		...
		midx = pFarsite->GetPerimeter1Value(NewFire, pFarsite->GetNumPoints(NewFire), ROSVAL);		// -1
		midy = pFarsite->GetPerimeter1Value(NewFire, pFarsite->GetNumPoints(NewFire), FLIVAL);		// 0
		//printf("<fswignt.cpp> xpt = %lf, ypt = %lf\n", xpt, ypt);
		//printf("<fswignt.cpp> midx = %lf, midy = %lf\n", midx, midy);
		do
		{
			OldNumPts = pFarsite->GetNumPoints(NewFire);
			//printf("<fsxwignt.cpp> OldNumPts = %ld\n", OldNumPts);
			DensityControl(NewFire);
			NumPts = pFarsite->GetNumPoints(NewFire);
			//printf("<fsxwignt.cpp> NumPts = %ld\n", NumPts);
