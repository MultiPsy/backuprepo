...
LandscapeTheme::LandscapeTheme(bool Analyze, Farsite5 *_pFarsite) : GridTheme()		// gridtheme으로 header 데이터 복사
{
	pFarsite = _pFarsite;
	long i;
	strcpy(Name, pFarsite->GetLandFileName());
	//cout << "Name = " << Name << endl;				// print out .lcp file name
...
void LandscapeTheme::ReadStats()
{
	...
	//for(int i = 0; i < 10; i++)	{
	//	printf("NumAllCats[%d] = %ld\n", i, NumAllCats[i]);
	//}
	memcpy(&AllCats[0], pFarsite->Header.elevs, 100 * sizeof(long));			// list of elevation values
	...
	memcpy(&AllCats[4], pFarsite->Header.covers, 100 * sizeof(long));			// list of canopy cover values
	memcpy(&AllCats[5], pFarsite->Header.heights, 100 * sizeof(long));			// list of canopy height values
	memcpy(&AllCats[6], pFarsite->Header.bases, 100 * sizeof(long));			// list of canopy base height values
	memcpy(&AllCats[7], pFarsite->Header.densities, 100 * sizeof(long));		// list of canopy bulk density values
	memcpy(&AllCats[8], pFarsite->Header.duffs, 100 * sizeof(long));			// list of duff values
	memcpy(&AllCats[9], pFarsite->Header.woodies, 100 * sizeof(long));			// list of coarse woody values
...
void LandscapeTheme::CopyStats(long layer)				// copy landscapetheme's fuel model to gridtheme
{
	//printf("CopyStats(3) -> copy landscapetheme's fuel models to gridtheme\n");
	memcpy(Cats, AllCats[layer], 99 * sizeof(long));	// AllCats[3] = fuel models -> copy to Cats
	NumCats = NumAllCats[layer];						// copy number of classes to NumCats
	MaxVal = maxval[layer];
	MinVal = minval[layer];	
