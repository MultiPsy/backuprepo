...
{
	//printf("<fsxwburn4.cpp> RunBurnThread()()---------------------------------------------\n");
...
{
	//printf("<fsxwburn4.cpp> PermimeterThread()---------------------------------------------\n");
	long begin, end;
	...
	res = pFarsite->Header.XResol;

	begin = Begin;
	end = End;
	int cnt = 0;
	//printf("PerimeterThread() ------------ End = %ld\n", End);
	...
	do	{
		...
		for (CurrentPoint = begin; CurrentPoint < end; CurrentPoint++)	{ // FOR ALL POINTS ON EACH FIRE
			...
			//printf("%ld\n", fe->GetLandscapeData(xpt, ypt, ld));
			cellx = (long)((xpt-pFarsite->Header.loeast)/res);
			celly = (long)((ypt-pFarsite->Header.lonorth)/res);
			printf("%ld, %ld, ", cellx, celly);			// ignition polygon perimeter cell positions
			cnt++;
			//printf("[ elev: %d, slope: %d, aspect: %d, fuel: %d, cover: %d ]\n", ld.elev, ld.slope, ld.aspect, ld.fuel, ld.cover);

			//--------------------------------------------------------------//
			// how to print out the coordinates' cell positions??? (solved) //
			// how to divide the coordinates based on grid??? (solved)      //
			// how to measure the computing time of each grid??? (file)     //
			//--------------------------------------------------------------//
			...
			if (turn == 1)	{
				...
				//printf("perimeters xpt = %lf, ypt = %lf\n", xpt, ypt);
				...
				if (pFarsite->DistanceCheckMethod(GETVAL) == 0 && pFarsite->CheckPostFrontal(GETVAL))	// store current fire perim in ring
				{
					...
					pFarsite->GetCurrentFuelMoistures(ld.fuel, ld.woody,
						(double *) &gmw, mx, 7);
					pFarsite->AddToCurrentFireRing(firering, CurrentPoint, ld.fuel, ld.woody, ld.duff, mx, cf.CrownLoadingBurned * 1000.0);
				}
			}
			clock_gettime(CLOCK_MONOTONIC, &Lfinish);
			printf("%.3lf\n", (Lfinish.tv_sec - Lstart.tv_sec) + (Lfinish.tv_nsec - Lstart.tv_nsec) / 1000.0);
			//printf("/10000000\n\n");
			pFarsite->do_whileCnt++;
		}
		//printf("%d\n\n", cnt);
//		SetFarsiteEvent(EVENT_BURN, ThreadOrder);		//hSimEvent[ThreadOrder]
		//printf("if(End<0)2 --------- %ld\n", End);
		if (End < 0)
			break;
	...
	//printf("out of PerimeterThread() do while() -------------\n");
...
{
	//printf("<fsxwburn4.cpp> SurfaceFire()---------------------------------------------\n");
...
{
	//printf("<fsxwburn4.cpp> CrownFire()---------------------------------------------\n");
...
void Burn::BurnIt(long count)
{
    ...
    //printf("End of BurnIt function\n");
	clock_gettime(CLOCK_MONOTONIC, &end);

	//cout << "BurnIt function elapsed time:" << (end.tv_sec - begin.tv_sec) + (end.tv_nsec - begin.tv_nsec) / 10000000000.0 << endl;
	//cout << "BurnIt function elapsed time:" << (end.tv_sec - begin.tv_sec) + (end.tv_nsec - begin.tv_nsec) / 10000000000.0 << endl;
}
...
{
	//printf("turn != 0, ResumePerimeterThreads() ----------------------\n");
	long i;

	//printf("ResumePerimeterThreads() turn = %ld\n", turn);
	for (i = 0; i < threadct; i++)
...
{
	//printf("turn = 0, StartPerimeterThreads_Equal()----------------\n");
	long i;
	...
	for (i = 0; i < NumPerimThreads; i++)	{
		...
		//printf("StartPerimeterThreads_Equal()---------------- turn = %ld\n", turn);
		burnthread[i]->SetRange(CurrentFire, SIMTIME, CuumTimeIncrement,
...
long Burn::StartPerimeterThreads_ActiveOnly()
{
	...
	for (i = 0; i < NumPerimThreads; i++)
	{
		...
		printf("StartPerimeterThreads_ActiveOnly() ------------ turn = %ld\n", turn);
		burnthread[i]->SetRange(CurrentFire, SIMTIME, CuumTimeIncrement,
						TimeIncrement, TimeMaxRem, begin, end, turn,
						FireIsUnderAttack, firering);
		threadct++;
...
{
	//printf("<fsxwburn4.cpp> BurnMethod1()---------------------------------------------\n");
...
Attack Atk(pFarsite);						// Instance of Attack
	...
	//printf("<fsxwburn4.cpp> BurnMethod1() TimeIncrement = %lf\n", TimeIncrement);
	CuumTimeIncrement = 0.0;
	...
	if ((atk = pFarsite->GetAttackForFireNumber(CurrentFire, 0, &NumLastAttack)) != 0)	{
		//printf("<fsxwburn4.cpp> BurnMethod1() FireIsUnderAttack = true\n");
 		FireIsUnderAttack = true;
	}
	else	{
		//printf("<fsxwburn4.cpp> BurnMethod1() FireIsUnderAttack = false\n");
		FireIsUnderAttack = false;
	}

 	if ((pFarsite->GetGroupAttackForFireNumber(CurrentFire, 0, &NumLastAttack)) != 0) 	{
		//printf("<fsxwburn4.cpp> BurnMethod1() GroupDirectAttack = true\n");
	  	GroupDirectAttack = true;
		gaat = new GroupAirAttack(pFarsite);
	}
	else	{
		//printf("<fsxwburn4.cpp> BurnMethod1() GroupDirectAttack = false\n");
		GroupDirectAttack = false;
		gaat = 0;}
	 //pFarsite->WritePerimeter1Shapefile(perim2Num++, CurrentFire);
  	do	{
		//printf("BurnMethod1() do{}\n");
	  	ExNumPts = 0;
		if (pFarsite->GetInout(CurrentFire) == 3 && !pFarsite->LEAVEPROCESS)	{
			CrossFires(0, &CurrentFire);
			//printf("<fsxwburn4.cpp> BurnMethod1() CrossFires()\n");
			CanStillBurn = true;
			//printf("do while() ---------------- TimeIncrement = %lf\n", TimeIncrement);
			TimeIncrement = 0.0;
		...
		//printf("turn = %ld\n", turn);
		if (turn == 0)	{				// what is turn??
			//printf("<fsxwburn4.cpp> BurnMethod1() turn == 0\n");
	...
	// 			env->CheckMoistureTimes(SIMTIME + CuumTimeIncrement);
			...
			//printf("ThreadCount = %ld\n", ThreadCount);
			//printf("turn = %ld\n", turn);
			//printf("<fsxwburn4.cpp> BurnMethod1() ThreadCount = %ld\n", ThreadCount);
		}
	  	else if(!pFarsite->LEAVEPROCESS)	{
			//printf("<fsxwburn4.cpp> BurnMethod1() !pFarsite->LEAVEPROCESS\n");
			//cout << "<fsxwburn4.cpp> BurnMethod1() LEAVEPROCESS = " << pFarsite->LEAVEPROCESS << endl;
		...
		{
			//printf("\n\npFarsite->GetNumPoints(CurrentFire) = %ld\n\n\n", pFarsite->GetNumPoints(CurrentFire));
...
//				if (InFiresBurned <= NumInFires)  //JAS! was strictly <  JAS!
				...
				if (TimeIncrement > 0.0)
				{
					...
					INFIRE = true;
					//printf("if do while() ----------- turn = 0\n");
				}
			}
			while (TimeIncrement == 0.0 && !pFarsite->LEAVEPROCESS);
		}
		//printf("TimeIncrement = %lf\n", TimeIncrement);
//        chint=getch();  // JAS!
		//printf("<fsxwburn4.cpp> BurnMethod1() TimeIncrement = %lf\n", TimeIncrement);
		//cout << "<fsxwburn4.cpp> BurnMethod1() pFarsite->LEAVEPROCESS = " << pFarsite->LEAVEPROCESS << endl;
		//printf("<fsxwburn4.cpp> FarsiteProcess1() docnt = %d\n", ++docnt);
		//printf("loopcnt = %d\n\n", ++loopcnt);
	}
